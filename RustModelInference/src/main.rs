...
fn main() {
    ...
    if !model_path.is_empty() && !prompt.is_empty() {
        if dump_logits {
            run_dump_logits(&model_path, &prompt, max_tokens, n_threads);
        } else {
            run_inference(&model_path, &prompt, max_tokens, temperature, n_threads, bench, profile);
        }
...
fn run_dump_logits(model_path: &str, prompt: &str, max_tokens: usize, n_threads_arg: usize) {
    ...
    let k_cache = vec![0u16; n_layer * max_ctx * n_embd_gqa];
    let v_cache = vec![0u16; n_layer * max_ctx * n_embd_gqa];
...
fn run_inference(model_path: &str, prompt: &str, max_tokens: usize, temperature: f32, n_threads_arg: usize, bench: bool, profile: bool) {
    let t0 = Instant::now();
...
fn run_interactive(model_path: &str, max_tokens: usize, temperature: f32, n_threads_arg: usize) {
    ...
    loop {
        ...
        run_inference(model_path, line, max_tokens, temperature, n_threads_arg, false, false);
        println!();
    }
}
