...
import { Wifi, Globe, RefreshCw, ArrowUp, ArrowDown, Activity, Server } from 'lucide-react'
...
export const Network = () => {
  ...
  const [tab, setTab] = useState<'interfaces' | 'connections' | 'traffic'>('interfaces')
  ...
  const fetchNetwork = async () => {
    ...
    setSpeeds(newSpeeds)
    setInfo(data)
    if (!selectedIface && Object.keys(data.interfaces).length > 0) {
      setSelectedIface(Object.keys(data.interfaces)[0])
    }
    setIsLoading(false)
  }
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          <div className="flex gap-1 bg-slate-100 rounded-lg p-1">
            {(['interfaces', 'connections', 'traffic'] as const).map(t => (
              <button
                key={t}
                onClick={() => setTab(t)}
              ...
              >
                {t === 'interfaces' ? '网卡' : t === 'connections' ? '连接' : '流量'}
              </button>
            ))}
          </div>
